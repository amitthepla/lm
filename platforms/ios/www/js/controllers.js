...
.controller('SideMenuCtrl', function($scope,$window,$ionicSideMenuDelegate,$filter,$state,CartCountFactory,$base64,getUserTypeFactory,LogoutUserDeviceIdFactory) {
	...
	$scope.home = function() {
		window.localStorage.removeItem("type_of_program");
	...
	$scope.logout = function() {
		var device_token_logout = window.localStorage.getItem("device_token");
		
		if(!device_token_logout || device_token_logout == null || device_token_logout == ''){
			updated_device_token = '5b547eafd8cf6e8d830a33fe7767ecbe73576e52b1aa01a08541909210c1778b';
		}else{
			updated_device_token = device_token_logout;
		}

		var orgId = window.localStorage.getItem("org_id");
		var auth_token = window.localStorage.getItem("auth_token");
		var result = LogoutUserDeviceIdFactory.LogoutUserDevice($base64.encode(orgId),$base64.encode(auth_token),updated_device_token);
		result.success(function(data){
			if(data.status == 'success'){
...
.controller('LoginCtrl', function($scope,$ionicModal, $timeout, ngFB,$ionicPlatform,$location,$timeout,$window,$ionicPopup,LoginFactory,$http,LoginFbFactory,$cordovaOauth) {
    ...
    $scope.doLogin = function() {
        if ($scope.LoginForm.$valid) {
				...
				if($scope.loginData) {
					...
					if(!device_token || device_token == null || device_token == ''){
						updated_device_token = '5b547eafd8cf6e8d830a33fe7767ecbe73576e52b1aa01a08541909210c1778b';
					}else{
						updated_device_token = device_token;
					}
					
					var result = LoginFactory.userLogin($scope.loginData,updated_device_token,device_type);
					result.success(function(data) {
    ...
    function getFbUserData(access_token,expires_in) {
        $http.get("https://graph.facebook.com/v2.2/me", {params: {access_token: access_token, fields: "id,name,email,birthday,first_name,last_name,gender", format: "json" }}).then(function(result) {
            ...
            var record = LoginFbFactory.userLoginFb(access_token,expires_in,user,updated_device_token,device_type);
...
.controller('ClubCtrl', function($scope,$ionicPopover,$ionicHistory,$timeout,$ionicSideMenuDelegate,$window,orgIdFactory,CartCountFactory,$stateParams,$filter,$base64,getUserTypeFactory,LogoutUserDeviceIdFactory) {
...
.controller('PostponeEventCtrl', function($scope,$filter,$ionicPopup,$window,eventDetailFactory,ButtonStatusFactory,$base64,cancelEventFactory,EditEventDateFormat,ConvertTimeFormat,PrefillDateFormat,PrefillTimeFormat) {
			...
			if($scope.action == 'reschedule') {
				...
				if(str) {
					...
					$.ajax({
						...
						data: JSON.parse(str),
						success: function(data){//console.log(data);
							window.localStorage['EventPushNotifyId'] = data.event_id;
							var alertPopup = $ionicPopup.alert({
								title: 'Success',
								cssClass : 'error_msg',
								template: data.message
	...
	function disable_enable_btn() {
		...
		response.success(function(data) {//console.log(data);
			$scope.isDisable = false;
			$("#content-loader").hide();
			if(data.social_links.length) {
				if(data.social_links['0'].facebook_connected == true) {
					$("#facebook").prop('checked', true);
				}
				if(data.social_links['0'].twitter_connected == true) {
...
.controller('RegistrationCtrl', function($scope,$ionicPopover,$ionicPopup,$timeout,RegOptFactory,$ionicSideMenuDelegate,$window,RegistrationFactory,$base64,allRegdFactory,$filter,ActivityTypeFactory,$ionicScrollDelegate) {
	var orgId = window.localStorage.getItem("org_id");
	var auth_token = window.localStorage.getItem("auth_token");
	$scope.isMessage = false;
	$scope.count = 0;
	$scope.init = function () {
		...
		if(window.localStorage.getItem('ProgramActivityId')) {
			$scope.show_selected_program(window.localStorage.getItem('ProgramActivityId'));
		} else {
			if(window.localStorage.getItem('type_of_program') == 'filter_activity') {
				$scope.show_filter_activity();
			} else {
				$scope.show_all_Activity();
			}
		}
	}
	$scope.toggleLeft = function() {
	...
	$scope.show_all_Activity = function() {
		$("#content-loader").show();
		$ionicScrollDelegate.$getByHandle('small').scrollTop();
		$("#program_div").hide();
		$("#program_type_div").hide();
		$("#menu_icon").show();
		$("#back_icon").hide();
		$("#search_icon").show();
		window.localStorage["FilterActivity"] = 0;
		$("#all_activity").addClass("active");
		$("#filter_activity").removeClass("active");
		window.localStorage['type_of_program'] = 'all_activity';
		window.localStorage.removeItem("ProgramActivityId");
		$scope.isMessage = false;
		$scope.isNoType = false;
	...
	$scope.show_filter_activity = function() {
		$("#content-loader").show();
        $ionicScrollDelegate.$getByHandle('small').scrollTop();
		$("#program_div").hide();
		$("#program_type_div").hide();
		...
		$("#back_icon").hide();
		$("#search_icon").hide();
		$("#filter_activity").addClass("active");
		$("#all_activity").removeClass("active");
		window.localStorage['type_of_program'] = 'filter_activity';
		window.localStorage.removeItem("ProgramActivityId");
		$scope.isMessage = false;
		$scope.isNoType = false;
		var record = ActivityTypeFactory.activityTypeList($base64.encode(orgId),$base64.encode(auth_token));
	...
	$scope.show_selected_program = function(activityId) {
		$("#content-loader").show();
        $ionicScrollDelegate.$getByHandle('small').scrollTop();
		$("#program_div").hide();
		$("#program_type_div").hide();
		...
		$("#filter_activity").addClass("active");
		$("#all_activity").removeClass("active");
		window.localStorage["FilterActivity"] = 1;
		$scope.isMessage = false;
		$scope.isNoType = false;
		var result1 = RegistrationFactory.getRegistrationRecord($base64.encode(orgId),$base64.encode(auth_token),activityId);
		result1.success(function(data) {
			...
			$("#program_type_div").hide();
			$("#content-loader").hide();
			window.localStorage["ProgramActivityId"] = activityId;
		});
		result1.error(function(data) {
			$("#content-loader").hide();
		});
	}
})
...
.controller('IndividualCtrl', function($scope,$ionicHistory,$stateParams,$timeout,TnCFactory,calculateAgeService,TnCAcceptFactory,UserInfoFactory,$ionicSideMenuDelegate,$window,RegdRec,IndividualRegFactory,$base64,$ionicPopup,$http,EditIndividualRegFactory,changeDateFormat,EditChangeDateFormat) {
	...
	function assignEditValue(data){
		var SaveEditIdValue = window.localStorage.getItem("SaveEditCardId");//alert(SaveEditIdValue);
		var AllRegDetails = {};
			var PaymentOptions = {};
			var WhomRegister = {};
			var skilllists = {};
			...
			if(data.registration_details.non_participation.valid && data.registration_details.is_team){ //TEAM Condition check for displaying the register for section
				...
				$(".Displaydob").show();
				/*For friend request*/
				if(data.registration_details.show_friend_requests) {
					$(".DisplayFriendAddForm").show();
					if(data.registration_details.friend_request_details.is_under_age) {
						$("#note_msg_div").show();
						$("#note_msg_div").text(data.registration_details.friend_request_details.under_age_message);
					}
				}
				
				if(SaveEditIdValue) {
					var friend_requests = data.registration_details.friend_request_details.friend_request_people_underage;
					for(var i in friend_requests) {
						if(i == 0) {
							$("#friend_request_0_email").val(friend_requests[i].emails[0].address);
							$("#friend_request_0_fname").val(friend_requests[i].first_name);
							$("#friend_request_0_lname").val(friend_requests[i].last_name);
						} else {
							var MoreFriend = '<label class="item item-input"><input type="text" class="FriendEmailCls" placeholder="Email Address" id="friend_request_'+i+'_email" value="'+friend_requests[i].emails[0].address+'"></label><div style="width:98%;"><label class="item item-input" style="float:left;width:48%"><input type="text" placeholder="First Name" class="FriendFnameCls" id="friend_request_'+i+'_fname" value="'+friend_requests[i].first_name+'"></label><label class="item item-input" style="float:right;width:48%;margin-right:2px;"><input type="text" class="FriendLnameCls" placeholder="Last Name" id="friend_request_'+i+'_lname" value="'+friend_requests[i].last_name+'"></label></div><div style="clear:both;heigh:10px;"></div>';
							$(".AddMoreFriendSection").append(MoreFriend);							
						}
					}
				}
				/*End friend request*/
				...
				if(data.registration_details.team_details.details.show_team_custom_fields){
					...
					angular.forEach(data.registration_details.team_details.details.team_custom_fields, function (teamcustomlist, key){
						if(teamcustomlist.show_field){
							...
							var teamcustomfieldIDValue = teamcustomlist.field_name;
							teamcustomfieldIDValue = teamcustomfieldIDValue.replace(/ /g, "_");
							teamcustomfieldIDValue = teamcustomfieldIDValue.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "_");
							if(teamcustomlist.type == "select"){
								teamcustomfieldlists += '<div class="fnt-bld" style="margin-top:10px;">'+teamcustomlist.field_name+displayTeamValidateStarMark+'</div><div class="list" style="margin-bottom:0px;"><select id="registration_custom_field_data_'+teamcustomfieldIDValue+'" class="item item-input item-select selectlabel getTeamDataClass '+TeamValidationClass+'" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']" '+TeamValidationFieldType+'>';
								...
								angular.forEach(teamcustomlist.values, function (value2, key2){
									...
									teamcustomfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt">'+CustomeMultiSelectvalue+'</div><div class="fr sche-tog"><label class="toggle toggle-balanced"><input id="registration_custom_field_data_'+teamcustomfieldIDValue+'_'+value2.replace(/ /g, "_")+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']['+value2+']" type="checkbox" class="chk-day '+teamcustomlist.field_name.replace(/ /g, "_")+'" value="'+value2+'"/><div class="track"><div class="handle"></div></div></label></div><div class="cb"></div></div>';
								});
								...
								angular.forEach(teamcustomlist.values, function (value3, key3){
									teamcustomfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt"><input type="radio" id="registration_custom_field_data_'+teamcustomfieldIDValue+'_'+value3.replace(/(<|>)/g, "_")+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']" class="rad-chk '+teamcustomlist.field_name.replace(/ /g, "_")+'" value="'+value3+'"/> <code>'+value3+'</code></div><div class="cb"></div></div>';
								});
								teamcustomfieldlists += '</div>';
							}else if(teamcustomlist.type == "text_field"){
								teamcustomfieldlists += '<div class="mt10"><div class="fnt-bld mb10">'+teamcustomlist.field_name+displayTeamValidateStarMark+'</div><div class="mt10 mb10"><label class="item item-input"><input type="text" placeholder="Textbox" id="registration_custom_field_data_'+teamcustomfieldIDValue+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']" class="getTeamDataClass" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'" '+TeamValidationFieldType+'></label></div></div>';
							}else if(teamcustomlist.type == "text_area"){
								teamcustomfieldlists += '<div class="mt10"><div class="fnt-bld mb10" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'">'+teamcustomlist.field_name+displayTeamValidateStarMark+'</div><div class="mt10 mb10"><label class="item item-input"><textarea customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'" id="registration_custom_field_data_'+teamcustomfieldIDValue+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']" class="getTeamDataClass '+TeamValidationClass+'" '+TeamValidationFieldType+'"></textarea></label></div></div>';
							}else if(teamcustomlist.type == "check_box"){
								if(teamcustomlist.values) {
									teamcustomfieldlists += '<div class="mt10"><div class="fnt-bld mb10 getTeamDataClass" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'">'+teamcustomlist.field_name+displayTeamValidateStarMark+'</div>';
									angular.forEach(teamcustomlist.values, function (value4, key4){
										teamcustomfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt"><input type="checkbox" id="registration_custom_field_data_'+teamcustomfieldIDValue+'_'+value4.replace(/(<|>)/g, "_")+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']"  class="chknoxcls" value="'+value4+'"/> <code>'+value4+'</code></div><div class="cb"></div></div>';
									});
								} else {
									var NewCustomFieldsList = teamcustomlist.field_name.replace(/ /g, "_");
									teamcustomfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'">'+teamcustomlist.field_name+displayTeamValidateStarMark+'</div><div class="fr sche-tog"><label class="toggle toggle-balanced"><input id="registration_custom_field_data_'+teamcustomfieldIDValue+'" name="registration[custom_field_data]['+teamcustomlist.field_name+']" type="checkbox" customFieldname="'+teamcustomlist.field_name.replace(/ /g, "_")+'" class="chk-day getTeamDataClass '+TeamValidationClass+'" '+TeamValidationFieldType+' value="1"/><div class="track"><div class="handle"></div></div></label></div><div class="cb"></div></div>';
								}							
								teamcustomfieldlists += '</div>';
							}
			...
			if(data.registration_details.show_custom_fields){
				...
				angular.forEach(data.registration_details.custom_fields, function (customlist, key){
					...
					if(customlist.show_field){
						...
						if(customlist.field_name != null) {
							var customfieldIDValue = customlist.field_name;
							customfieldIDValue = customfieldIDValue.replace(/ /g, "_");
							customfieldIDValue = customfieldIDValue.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "_");
						}
						if(customlist.type == "select"){
							...
							customfieldIDValue = customfieldIDValue.replace(/,/g, "_");
							customfieldIDValue = customfieldIDValue.replace(/\(|\)/g, "_");*/
							customfieldlists += '<div class="fnt-bld" style="margin-top:10px;">'+customlist.field_name + displayValidateStarMark+'</div><div class="list" style="margin-bottom:0px;"><select id="registration_custom_field_data_'+customfieldIDValue+'" class="item item-input item-select selectlabel getDataClass '+ValidationClass+'" customFieldname="'+customlist.field_name.replace(/ /g, "_")+'" name="registration[custom_field_data]['+customlist.field_name+']" '+ValidationFieldType+'>';
							...
							customfieldIDValue = customfieldIDValue.replace(/ /g, "_");
							customfieldIDValue = customfieldIDValue.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "_");*/
							customfieldlists += '<div class="mt10"><div class="fnt-bld mb10">'+customlist.field_name+displayValidateStarMark+'</div><div class="mt10 mb10"><label class="item item-input"><input type="text" placeholder="Textbox" id="registration_custom_field_data_'+customfieldIDValue+'" name="registration[custom_field_data]['+customlist.field_name+']" class="getDataClass '+ValidationClass+'" customFieldname="'+customlist.field_name.replace(/ /g, "_")+'" '+ValidationFieldType+'></label></div></div>';
						}else if(customlist.type == "text_area"){
							customfieldlists += '<div class="mt10"><div class="fnt-bld mb10" customFieldname="'+customlist.field_name.replace(/ /g, "_")+'">'+customlist.field_name+displayValidateStarMark+'</div><div class="mt10 mb10"><label class="item item-input"><textarea  customFieldname="'+customlist.field_name.replace(/ /g, "_")+'" id="registration_custom_field_data_'+customfieldIDValue+'" name="registration[custom_field_data]['+customlist.field_name+']" class="getDataClass '+ValidationClass+'" '+ValidationFieldType+'"></textarea></label></div></div>';
						}else if(customlist.type == "check_box"){
							
							if(customlist.values) {
								customfieldlists += '<div class="mt10"><div class="fnt-bld mb10 getDataClass '+ValidationClass+'" '+ValidationFieldType+'">'+customlist.field_name+displayValidateStarMark+'</div>';
								angular.forEach(customlist.values, function (value4, key4){
									customfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt"><input type="checkbox" id="registration_custom_field_data_'+customfieldIDValue+'_'+value4.replace(/(<|>)/g, "_")+'" name="registration[custom_field_data]['+customlist.field_name+']"  class="chknoxcls" value="'+value4+'"/> <code>'+value4+'</code></div><div class="cb"></div></div>';
								});
							} else {
								//customfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt"><label class="toggle toggle-balanced"><input type="checkbox" id="registration_custom_field_data_'+customlist.field_name.replace(/ /g, "_")+'" name="registration[custom_field_data]['+customlist.field_name+']"  class="chknoxcls" value=""/> <code></code></label></div><div class="cb"></div></div>';
								var NewCustomFieldsList = customlist.field_name.replace(/ /g, "_");
								customfieldlists += '<div class="mt10 mb10"><div class="fl nrm-fnt" customFieldname="'+customlist.field_name.replace(/ /g, "_")+'">'+customlist.field_name+displayValidateStarMark+'</div><div class="fr sche-tog"><label class="toggle toggle-balanced"><input id="registration_custom_field_data_'+customfieldIDValue+'" name="registration[custom_field_data]['+customlist.field_name+']" type="checkbox" customFieldname="'+customlist.field_name.replace(/ /g, "_")+'" class="chk-day getDataClass '+ValidationClass+'" '+ValidationFieldType+' value="1"/><div class="track"><div class="handle"></div></div></label></div><div class="cb"></div></div>';
							}
							customfieldlists += '</div>';
						}
				...
				if(data.registration_details.is_team){ //Condition for showing the team details
					...
					angular.forEach(data.registration_details.team_details.details.team_custom_fields, function (customlist, key){
						if(customlist.show_field){
							...
							if(customlist.field_name){
								var CustomFieldEditValue = customlist.field_name;
								CustomFieldEditValue = CustomFieldEditValue.replace(/ /g, "_");
								CustomFieldEditValue = CustomFieldEditValue.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "_");
							}else{
								var CustomFieldEditValue = "";
							}
							
							if(customlist.type == "select"){
								if(customlist.selected_value){
									$('#registration_custom_field_data_'+CustomFieldEditValue+' option[value="'+customlist.selected_value+'"]').prop('selected', true);
								}
							}else if(customlist.type == "multiselect"){
								if(customlist.selected_value){
									var AllDayField = customlist.selected_value.split(",");
									for(var i=0;i<AllDayField.length;i++){
										$("#registration_custom_field_data_"+CustomFieldEditValue+"_"+AllDayField[i].trim().replace(/ /g, "_").replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "_")).prop('checked',true);
									}
								}
							}else if(customlist.type == "radio_buttons"){
								if(customlist.selected_value){
									$("#registration_custom_field_data_"+CustomFieldEditValue+"_"+customlist.selected_value.replace(/(<|>)/g, '_')).attr('checked','checked');
								}
							}else if(customlist.type == "text_field"){
								if(customlist.selected_value){
									$("#registration_custom_field_data_"+CustomFieldEditValue).val(customlist.selected_value);
								}
							}else if(customlist.type == "text_area"){
								if(customlist.selected_value){
									$("#registration_custom_field_data_"+CustomFieldEditValue).val(customlist.selected_value);
								...
								if(customlist.selected_value == 'Yes'){
									$("#registration_custom_field_data_"+CustomFieldEditValue).attr('checked','checked');
								}
							}
						}
					});
...
//		$scope.is_accept = TnCAcceptFactory.getAccept();
	...
	$scope.selectRoster = function(){
		...
		if(rosterOpt == 'Yes'){
			...
			$(".AddressCls").show();
			$(".AddInfos").show();
			if($scope.totalData.registration_details.show_emergency_contact){
				$(".EmergContact").show();
			}
			$(".displayCustomIndi").show();
			...
			if(LoggedInUserDetail.registration_details.emergency_contact_person.first_name){
				$("#emgFirstName").val(LoggedInUserDetail.registration_details.emergency_contact_person.first_name);
			}
			if(LoggedInUserDetail.registration_details.emergency_contact_person.last_name){
				$("#emgLastName").val(LoggedInUserDetail.registration_details.emergency_contact_person.last_name);
			}
			if(LoggedInUserDetail.registration_details.emergency_contact_person.address){
				$("#emgEmail").val(LoggedInUserDetail.registration_details.emergency_contact_person.address);
			}
			if(LoggedInUserDetail.registration_details.emergency_contact_person.phone){
				$("#emgPhone").val(LoggedInUserDetail.registration_details.emergency_contact_person.phone);
			}
...
//	}
	...
	$scope.AddMoreFriend = function() {
		var x = $('.FriendEmailCls').length;
		var MoreFriend = '<label class="item item-input"><input type="text" class="FriendEmailCls" placeholder="Email Address" id="friend_request_'+x+'_email"></label><div style="width:98%;"><label class="item item-input" style="float:left;width:48%"><input type="text" placeholder="First Name" class="FriendFnameCls" id="friend_request_'+x+'_fname"></label><label class="item item-input" style="float:right;width:48%;margin-right:2px;"><input type="text" class="FriendLnameCls" placeholder="Last Name" id="friend_request_'+x+'_lname"></label></div><div style="clear:both;heigh:10px;"></div>';
		$(".AddMoreFriendSection").append(MoreFriend);
	}
	
	function getFriendInfo() {
		if($('.DisplayFriendAddForm').is(':visible')) {
			var x = $('.FriendEmailCls').length;
			var FriendArr = '"friend_request_people_attributes":{';
			var count = 0;
			$(".FriendEmailCls").each(function(){
				var Frndmail = $(this).val();
				if(Frndmail) {	
					var reg = /^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})$/;
					if(!reg.test(Frndmail)){
						var alertPopup = $ionicPopup.alert({
						   title: '<div style="text-align:center;">Friend request email address is invalid</div>'
						});
						return false;
					}
				} 
				var Frndfname = $("#friend_request_"+count+"_fname").val();
				var Frndlname = $("#friend_request_"+count+"_lname").val();
				if(x == count+1) {
					FriendArr += '"'+count+'":{"emails_attributes":{"0":{"address":"'+Frndmail+'"}},"first_name":"'+Frndfname+'","last_name":"'+Frndlname+'"}';
				} else {
					FriendArr += '"'+count+'":{"emails_attributes":{"0":{"address":"'+Frndmail+'"}},"first_name":"'+Frndfname+'","last_name":"'+Frndlname+'"},';
				}
				count++;
				...
				$(".ClassValidation").each(function() {
					...
					if(validtypecheck == "select"){
						...
						if($(this).val()){
						   ...
						   var alertPopup3 = $ionicPopup.alert({
							   title: '<div style="text-align:center;">Please enter '+fieldName.replace(/_/g, " ")+'.</div>'
						   });
						   SubmitStatus = false;
						   return false;
						}
		...
		if(SubmitStatus == true){
			...
			if(isTeamSts=='false' && validSts=='false') //Submitting for the Individual Condition
			{
				...
				if(getDataCounter > 0){ //This condition is required to get the values for the Custom Field Types
					...
					$(".getDataClass").each(function() {
						...
						}else if(validtypecheck == "check_box"){
							var fieldId = $(this).attr("id");
							if($("#"+fieldId).is(":checked")) {
								var selectareaval = 'on';
							} else {
								var selectareaval = '';
							}			
							CustomFieldsArrtemp += '"'+fieldNameDis+'":"'+selectareaval+'",';
				...
				var frnd_data = getFriendInfo();
				if(frnd_data) {
					CustomFieldsArr += frnd_data+',';
				}
			...
			else if(isTeamSts=='true' && validSts=='true') //This is the condition for the Team registration
			{
				...
				if(RosterVal == "No"){
					...
					var isanycustomfielddatapresent = 0;
					var IndiCustomFieldData = '';
					...
					if(getTeamDataCounter > 0){ //This condition is required to get the values for the Custom Field Types
						...
						IndiCustomFieldData += '"custom_field_data":{';
						$(".getTeamDataClass").each(function() {
							...
							if(validtypecheck == "select"){
								...
								if(selectval){
									...
									IndiCustomFieldData += '"'+fieldNameDis+'":"'+selectval+'",';
									isanycustomfielddatapresent++;
								}
							}else if(validtypecheck == "multiselect"){
								if($("."+fieldNameId+":checkbox:checked").length > 0){
									//CustomFieldsArr += '"'+fieldNameDis+'":{';
									IndiCustomFieldData += '"'+fieldNameDis+'":{';
									$.each($("."+fieldNameId+":checkbox:checked"), function(){
									   //CustomFieldsArr += '"'+$(this).val()+'":"'+$(this).val()+'",';
									   IndiCustomFieldData += '"'+$(this).val()+'":"'+$(this).val()+'",';
									   isanycustomfielddatapresent++;
									});
									//CustomFieldsArr = CustomFieldsArr.substr(0, (CustomFieldsArr.length)-1)
									//CustomFieldsArr = CustomFieldsArr+'},';
									
									IndiCustomFieldData = IndiCustomFieldData.substr(0, (IndiCustomFieldData.length)-1)
									IndiCustomFieldData = IndiCustomFieldData+'},';
								}
							}else if(validtypecheck == "radio_buttons"){
								if($("."+fieldNameId+":radio:checked").length > 0){
									var radioval = $("."+fieldNameId+":checked").val();
									//CustomFieldsArr += '"'+fieldNameDis+'":"'+radioval+'",';
									IndiCustomFieldData += '"'+fieldNameDis+'":"'+radioval+'",';
									isanycustomfielddatapresent++;
								}
							}else if(validtypecheck == "text_field"){
								var fieldId = $(this).attr("id");
								...
								if(selecttextval){
									...
									IndiCustomFieldData += '"'+fieldNameDis+'":"'+selecttextval+'",';
									isanycustomfielddatapresent++;
								}
							}else if(validtypecheck == "text_area"){
								var fieldId = $(this).attr("id");
								...
								if(selectareaval){
									...
									IndiCustomFieldData += '"'+fieldNameDis+'":"'+selectareaval+'",';
									isanycustomfielddatapresent++;
								}
							}else if(validtypecheck == "check_box"){
								var fieldId = $(this).attr("id");
					...
					var rosterOpt = $("#registration_participating_roster").val();
					getDataCounter = $(".getDataClass").length;
					if(getDataCounter > 0 && rosterOpt=='Yes'){ //This condition is required to get the values for the Custom Field Types if the roster option is yes
						if(getTeamDataCounter == 0 && getDataCounter > 0){ //If there are no team custom fields but general custom fields are present
							//CustomFieldsArr += '"custom_field_data":{';
							IndiCustomFieldData = '"custom_field_data":{';
						}
						$(".getDataClass").each(function() {
					...
					var isCustomFieldsVisible = $('.displayCustomIndi').is(":visible");
					if(getTeamDataCounter > 0 || (getDataCounter > 0 && isCustomFieldsVisible)){
						//CustomFieldsArr = CustomFieldsArr.substr(0, (CustomFieldsArr.length)-1);
						//CustomFieldsArr = CustomFieldsArr+'},';
					...
					if(isanycustomfielddatapresent > 0){ //This condition is require if custom field presents and it has data in it
						CustomFieldsArr += IndiCustomFieldData
					}
					...
					if(Alldataregtarget.registration_details.registration_target_person_details.label)
					{
						...
						if(SaveEditIdValue){ //This condition is required to pass the data for the Edit condition
							CustomFieldsArr = CustomFieldsArr + '"registration_target_person_attributes":{"emails_attributes":{"0":{"address":"'+Alldataregtarget.registration_details.registration_target_person_details.emails[0].address+'","id":"'+Alldataregtarget.registration_details.registration_target_person_details.emails[0].id+'"}},"first_name":"'+Alldataregtarget.registration_details.registration_target_person_details.first_name+'","last_name":"'+Alldataregtarget.registration_details.registration_target_person_details.last_name+'","gender":"'+Alldataregtarget.registration_details.registration_target_person_details.gender+'","birthdate":"'+Alldataregtarget.registration_details.registration_target_person_details.birthdate+'","address":"'+street+'","city":"'+city+'","state":"'+state+'","zip":"'+zip+'","phone":"'+phone+'","id":"'+Alldataregtarget.registration_details.registration_target_person_details.id+'"},';
						}else{ //This condition is required for the add condition
							if(getDataCounter > 0 && rosterOpt=='Yes'){
								CustomFieldsArr = CustomFieldsArr + '"registration_target_person_attributes":{"emails_attributes":{"0":{"address":"'+Alldataregtarget.registration_details.registration_target_person_details.emails[0].address+'"}},"first_name":"'+Alldataregtarget.registration_details.registration_target_person_details.first_name+'","last_name":"'+Alldataregtarget.registration_details.registration_target_person_details.last_name+'","gender":"'+Alldataregtarget.registration_details.registration_target_person_details.gender+'","birthdate":"'+Alldataregtarget.registration_details.registration_target_person_details.birthdate+'","address":"'+street+'","city":"'+city+'","state":"'+state+'","zip":"'+zip+'","phone":"'+phone+'"},';
							}else{
								CustomFieldsArr = CustomFieldsArr + '"registration_target_person_attributes":{"emails_attributes":{"0":{"address":"'+Alldataregtarget.registration_details.registration_target_person_details.emails[0].address+'"}},"first_name":"'+Alldataregtarget.registration_details.registration_target_person_details.first_name+'","last_name":"'+Alldataregtarget.registration_details.registration_target_person_details.last_name+'","gender":"'+Alldataregtarget.registration_details.registration_target_person_details.gender+'","birthdate":"'+Alldataregtarget.registration_details.registration_target_person_details.birthdate+'","address":"'+street+'","city":"'+city+'","state":"'+state+'","zip":"'+zip+'","phone":"'+phone+'"},';
							}
						}
			...
			else if(isTeamSts=='true' && validSts=='false') //This is the condition for the Group registration
			{
				...
				var isanycustomfielddatapresentGRP = 0;
				var IndiCustomFieldDataGRP = '';
				var rosterOpt = $("#registration_participating_roster").val();
				getTeamDataCounter = $(".getTeamDataClass").length;
					if(getTeamDataCounter > 0){ //This condition is required to get the values for the Custom Field Types if the roster option is yes
						//CustomFieldsArr += '"custom_field_data":{';
						IndiCustomFieldDataGRP += '"custom_field_data":{';
						$(".getTeamDataClass").each(function() {
							var validtypecheck = $(this).attr("validfieldtype");
							var fieldNameId = $(this).attr("customfieldname");
							var fieldNameDis = fieldNameId.replace(/_/g, " ");
							if(validtypecheck == "select"){
								...
								if(selectval){
									//CustomFieldsArr += '"'+fieldNameDis+'":"'+selectval+'",';
									IndiCustomFieldDataGRP += '"'+fieldNameDis+'":"'+selectval+'",';
									isanycustomfielddatapresentGRP++;
								}
							}else if(validtypecheck == "multiselect"){
								if($("."+fieldNameId+":checkbox:checked").length > 0){
									//CustomFieldsArr += '"'+fieldNameDis+'":{';
									IndiCustomFieldDataGRP += '"'+fieldNameDis+'":{';
									$.each($("."+fieldNameId+":checkbox:checked"), function(){
									   //CustomFieldsArr += '"'+$(this).val()+'":"'+$(this).val()+'",';
									   IndiCustomFieldDataGRP += '"'+$(this).val()+'":"'+$(this).val()+'",';
									   isanycustomfielddatapresentGRP++;
									});
									//CustomFieldsArr = CustomFieldsArr.substr(0, (CustomFieldsArr.length)-1)
									//CustomFieldsArr = CustomFieldsArr+'},';
									
									IndiCustomFieldDataGRP = IndiCustomFieldDataGRP.substr(0, (IndiCustomFieldDataGRP.length)-1)
									IndiCustomFieldDataGRP = IndiCustomFieldDataGRP+'},';
								}
							}else if(validtypecheck == "radio_buttons"){
								if($("."+fieldNameId+":radio:checked").length > 0){
									var radioval = $("."+fieldNameId+":checked").val();
									//CustomFieldsArr += '"'+fieldNameDis+'":"'+radioval+'",';
									IndiCustomFieldDataGRP += '"'+fieldNameDis+'":"'+radioval+'",';
									isanycustomfielddatapresentGRP++;
								}
							}else if(validtypecheck == "text_field"){
								//var fieldId = $(this).attr("id");
								...
								if(selectval){
									//CustomFieldsArr += '"'+fieldNameDis+'":"'+selecttextval+'",';
									IndiCustomFieldDataGRP += '"'+fieldNameDis+'":"'+selecttextval+'",';
									isanycustomfielddatapresentGRP++;
								}
							}else if(validtypecheck == "text_area"){
								var fieldId = $(this).attr("id");
								...
								if(selectareaval){
									//CustomFieldsArr += '"'+fieldNameDis+'":"'+selectareaval+'",';
									IndiCustomFieldDataGRP += '"'+fieldNameDis+'":"'+selectareaval+'",';
									isanycustomfielddatapresentGRP++;
								}
							}else if(validtypecheck == "check_box"){
								var fieldId = $(this).attr("id");
					...
					getDataCounter = $(".getDataClass").length;
					if(getDataCounter > 0){ //This condition is required to get the values for the Custom Field Types if the roster option is yes
						if(getTeamDataCounter == 0 && getDataCounter > 0){ //If there are no team custom fields but general custom fields are present
							//CustomFieldsArr += '"custom_field_data":{';
							IndiCustomFieldDataGRP += '"custom_field_data":{';
						}
					...
					var isCustomFieldsVisible = $('.displayCustomIndi').is(":visible");
 					...
 					if(getTeamDataCounter > 0 || (getDataCounter > 0 && isCustomFieldsVisible)){ //If there custom fields then display them else not
						//CustomFieldsArr = CustomFieldsArr.substr(0, (CustomFieldsArr.length)-1);
						//CustomFieldsArr = CustomFieldsArr+'},';
					...
					if(isanycustomfielddatapresentGRP > 0){ //This condition is require if custom field presents and it has data in it
						CustomFieldsArr += IndiCustomFieldDataGRP
					}
			...
			//console.log(CustomFieldsArr);return false;
			if(CustomFieldsArr){
...
.controller('SiteNewsDetailCtrl',function($scope,$stateParams,$timeout,$ionicSideMenuDelegate,$window,$timeout,$ionicSlideBoxDelegate,$ionicModal,$ionicSlideBoxDelegate,$ionicScrollDelegate,getPushNewsFactory,$base64,$location) {
    var orgId = window.localStorage.getItem("org_id");
    var auth_token = window.localStorage.getItem("auth_token");
  	$scope.zoomMin = 1;
  	$scope.showImages = function(index) {
  	...
  	$scope.$on( "$ionicView.enter", function(scopes,states) {
  		$('#news_detail a').each(function() {
  			...
  			if(path != "" && path.indexOf("http:/") < 0) {
  				...
  				$(this).attr("url", "http:/"+path);
  			}
  		});
  		$("#content-loader").hide();
  	});
  	$(document).on('click','#news_detail a',function() {
  		window.open($(this).attr("url"), '_system', 'location=yes'); 
  		return false;
  	});
  	$(document).on('click','#url',function() {
  		window.open($("#url").text(), '_system', 'location=yes'); 
  		return false;
  	});
})
